...
from .models import Ejercicio, IntentoEjercicio, DetalleIntento, DetalleRetroalimentacion
...
class EjercicioSerializerCreate(serializers.ModelSerializer):
    ...
    class Meta:
        model = Ejercicio
        fields = ('id_ejercicio', 'fecha', 'id_usuario', 'enunciado_file', 'casos_de_uso_file', 'dificultad', 'contenidos', 'puntaje','lenguaje','asignatura')


class EjercicioSerializerView(serializers.ModelSerializer):
    class Meta:
        model = Ejercicio
        fields = ['id_ejercicio', 'fecha', 'id_usuario', 'dificultad', 'contenidos', 'puntaje','lenguaje']
...
class EjercicioListSerializerAll(serializers.ModelSerializer):
    class Meta:
        model = Ejercicio
        fields = ['id_ejercicio', 'dificultad', 'puntaje', 'contenidos','lenguaje','asignatura','id_usuario','nombre_usuario','email_usuario']
...
class IntentoEjercicioSerializer(serializers.ModelSerializer):
    ...
    id_usuario = serializers.PrimaryKeyRelatedField(queryset=get_user_model().objects.all())
    tiempo = serializers.CharField(default='10:00:00')
    codigo = serializers.CharField(default='print("Hello World")', write_only=True)
    feedback_inicial = serializers.CharField(default='feedback inicial', write_only=True)
