...
def run_code_in_container(code: str, timeout_seconds=20) -> str:
    ...
    try:
        ...
        exec_output = container.exec_run(cmd=f'sh -c "echo \\"{corrected_code_for_command}\\" > {temp_filename}"', user="appuser")
        ...
        exec_result = container.exec_run(cmd=f"timeout {timeout_seconds} python {temp_filename}", user="appuser")
        ...
        if exec_result.exit_code != 0:
            if exec_result.exit_code == 124:  # timeout cuando se supera el tiempo
                return "Error: El código superó el tiempo máximo permitido de ejecución."
